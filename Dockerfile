...
RUN curl -LO https://storage.googleapis.com/kubernetes-release/release/${KUBECTL_VERSION}/bin/darwin/amd64/kubectl   && chmod +x kubectl
...
RUN echo "installing ansible collection" && ansible-galaxy collection install -r requiremts-galaxy.yaml && rm requiremts-galaxy.yaml
